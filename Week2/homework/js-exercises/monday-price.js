...
let durations = mondayTasks.map(timePicker);
console.log("duration array is " + durations);

//Multiply each duration by a per-hour rate for billing and sum it all up.
function timeToCash(item) {
    totalMoney += item / 60 * hourlyRate;
}

durations.forEach(timeToCash);
console.log("total money is €" + Math.round(totalMoney * 100) / 100);